  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      if (memberData) {
        ...
        // 로그인 유저 프로필 이미지 찾기
        const currentUser = parsedMembers.find((member) => member.id === user.id);
      ...
      reader.onloadend = () => {
        ...
        if (memberData) {
          ...
          if (user) {
            ...
            const updatedMembers = parsedMembers.map((member) => (member.id === user.id ? { ...member, profileImage: base64Image } : member));
  ...
  useEffect(() => {
    if (user) {
      if (Array.isArray(user.bData) && user.bData.length > 0) {
        ...
        const newBadges = unlockedBadges.filter((badge) => !completedBadges.includes(badge.badgeTitle) && !setCelebratedBadges.includes(badge.badgeTitle));

        if (newBadges.length > 0) {
          // 새로운 뱃지가 하나라도 있으면 한 번만 실행
          setCompletedBadges((prevBadges) => [...prevBadges, ...newBadges.map((badge) => badge.badgeTitle)]);
          ...
          const updatedUser = {
            ...user,
            setCelebratedBadges: [...setCelebratedBadges, ...newBadges.map((badge) => badge.badgeTitle)],
          };
  ...
  useEffect(() => {
    if (user) {
      ...
      if (sessionUserData) {
        ...
        const existingMember = localStorageMemberData.find((member) => member.id === sessionUserData.id);

        // 유저 데이터가 다르거나, 새로운 데이터가 있을 경우 업데이트
        if (!existingMember) {
          // 새 유저 데이터를 로컬스토리지에 추가
          localStorage.setItem("member_data", JSON.stringify([...localStorageMemberData, sessionUserData]));
        } else {
          // 유저 데이터가 변경된 경우 (예: 뱃지 추가 등)
          const updatedMemberData = localStorageMemberData.map((member) => (member.id === sessionUserData.id ? { ...member, ...sessionUserData } : member));
          ...
          <div className="inner-section profile-wrap">
            <div className="profile-box">
              <div className="profile-image">
                ...
                />
                <input type="file" id="profile-image-input" style={{ display: "none" }} onChange={handleProfileImageChange} />
              </div>
            ...
            <div className="right">
              <div className="level-box" onClick={handleMoreClick}>
                <div className="level">
                  <img src={process.env.PUBLIC_URL + `/img/sub/img-level${level}.svg`} alt={`레벨 ${level}`} className="level-img" />
                ...
                <div className="more-modal">
                  <div className="modal-content">
                    <div ref={cardRef} className="card">
                      <img className="img" src={process.env.PUBLIC_URL + `/img/sub/img-level${level}.png`} />
                    </div>
                  ...
                  unlockedBadges.map((badge, index) => (
                    <li key={index} className={`badge-item ${badge.isActive ? "active" : ""}`}>
                      <div className="img-box">
                        <img src={process.env.PUBLIC_URL + badge.badgeSrc || process.env.PUBLIC_URL + "/img/sub/default-badge.png"} alt={badge.badgeAlt} />
                      </div>
